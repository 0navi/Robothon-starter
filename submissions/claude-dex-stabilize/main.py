...
DT = 0.002
FPS = 30
DURATION_S = 90.0   # 90 s (30 perturbations across 1.5 min) — meets the 1-3 min demo requirement
...
GRIP_TIGHTEN_DELTA = {
    "if_mcp": 0.25, "if_pip": 0.30, "if_dip": 0.20,
    "mf_mcp": 0.25, "mf_pip": 0.30, "mf_dip": 0.20,
    "rf_mcp": 0.25, "rf_pip": 0.30, "rf_dip": 0.20,
    "th_cmc": 0.15, "th_mcp": 0.25, "th_ipl": 0.20,
}
...
def _draw_drift_chart(d, drift_history_mm: list):
    ...
    for i, v in enumerate(window):
        ...
        pts.append((px, py))
    # baseline at 8mm threshold (where grip starts tightening)
    y_thresh = (y0 + h - 6) - (h - 28) * (8.0 / ymax)
    d.line([(x0 + 6, y_thresh), (x0 + w - 6, y_thresh)],
           fill=(120, 120, 120, 180), width=1)
    d.text((x0 + w - 60, y_thresh - 16), "8 mm",
    ...
    if render_video:
        ...
        summary = {
            ...
            "closed_loop": {
                ...
                "law": "tighten = clamp((drift - 8mm) / 22mm, 0, 1) applied as additive grip delta",
            },
